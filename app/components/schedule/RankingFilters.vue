    ...
    <v-card class="pa-2">
      ...
      <v-card-text ref="dialogContent" class="pt-4">
        <v-select
          ...
          variant="outlined"
          density="comfortable"
          @update:model-value="draft.freeDays = $event ?? []"
        />
        ...
        <v-row class="field-row">
          <v-col cols="12" sm="6">
            <v-text-field
              :model-value="draft.maxOccupiedDays"
              ...
              density="comfortable"
              @update:model-value="
                draft.maxOccupiedDays =
                  $event == null || $event === '' ? undefined : Number($event)
              "
            />
          </v-col>
          <v-col cols="12" sm="6">
            <v-select
              :model-value="draft.maxTotalGapMinutes"
              :items="gapOptions"
              ...
              density="comfortable"
              @update:model-value="
                draft.maxTotalGapMinutes = $event ?? undefined
              "
            />
          </v-col>
          <v-col cols="12" sm="6">
            <v-select
              :model-value="draft.maxSingleGapMinutes"
              :items="gapOptions"
              ...
              density="comfortable"
              @update:model-value="
                draft.maxSingleGapMinutes = $event ?? undefined
              "
            />
          </v-col>
          <v-col cols="12" sm="6">
            <v-select
              :model-value="draft.maxConsecutiveMinutes"
              ...
              density="comfortable"
              @update:model-value="
                draft.maxConsecutiveMinutes = $event ?? undefined
              "
            />
          </v-col>
        </v-row>
...
const props = defineProps<{
  modelValue: IScheduleRankingPreferences
  disabled?: boolean
}>()
...
const invalidDays = computed(
  () =>
    ...
    (!Number.isInteger(draft.value.maxOccupiedDays) ||
      draft.value.maxOccupiedDays < 1 ||
      draft.value.maxOccupiedDays > 7),
)
