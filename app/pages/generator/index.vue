<template>
  ...
  >
    ...
    <template #subtitle-items>
      ...
      />
      <v-btn v-if="loadingGenerate" variant="text" @click="cancelGeneration"
        >Cancelar generación</v-btn
      >
      ...
      <ScheduleRankingSummary
        ...
        :metrics="
          getScheduleRankingMetrics(
            item,
            result?.scheduleRanking ?? localScheduleRanking,
          )
        "
    ...
    <template #emptyBody>
      <template v-if="result">
        <v-alert
          v-if="generatedBeforeFilters === null"
          type="info"
          variant="tonal"
        >
          Esta generación guardada no contiene horarios. Regenera para revisar
          los cruces y los filtros actuales.
        </v-alert>
        <v-alert
...
const { loadSchedules, cancelGeneration } = useSchedulesGenerator()
...
const generatedBeforeFilters = ref<number | null>(null)
