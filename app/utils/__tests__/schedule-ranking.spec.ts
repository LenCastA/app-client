...
describe('schedule ranking', () => {
  it('calculates diagnostics beyond the JavaScript function argument limit', () => {
    const schedule = makeSchedule('1', [
      { day: 1, startTime: '08:00', endTime: '09:00' },
    ])
    const result = rankSchedules(
      Array.from({ length: 150_000 }, () => schedule),
      { ...preferences, freeDays: [1] },
    )
    expect(result.ranked).toHaveLength(0)
    expect(result.filteredOut).toBe(150_000)
    expect(result.diagnostics.minimumPossibleDays).toBe(1)
    expect(result.diagnostics.freeDays).toEqual([
      { day: 1, rejectedCount: 150_000 },
    ])
  })
